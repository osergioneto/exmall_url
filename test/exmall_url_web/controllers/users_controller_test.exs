defmodule ExmallUrlWeb.UsersControllerTest do
  ...
  describe "POST /users" do
    test "should return status 201 and create user when POST to route /api/users", %{conn: _conn} do
    ...
    test "should return status 400 and error when POST when params are incorret", %{conn: _conn} do
      %{resp_body: resp_body, status: status} =
        post(build_conn(), "/api/users",
          name: "Sérgio Neto",
          email: "not-mail",
